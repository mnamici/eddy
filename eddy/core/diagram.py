...
class Diagram(QtWidgets.QGraphicsScene):
    """
    Extension of QtWidgets.QGraphicsScene which implements a single generic diagram.
...
class GrapholDiagram(Diagram):
    """
    Extension of Diagram which implements a single Graphol diagram.
    Additionally to built-in signals, this class emits:

    * sgnNodeIdentification: whenever node identification is requested
    """
    ...
    def create(cls, name, size, project):
        ...
        :rtype: Diagram
        """
        diagram = GrapholDiagram(name, project)
        diagram.setSceneRect(QtCore.QRectF(-size / 2, -size / 2, size, size))
        diagram.setItemIndexMethod(Diagram.BspTreeIndex)
        return diagram
